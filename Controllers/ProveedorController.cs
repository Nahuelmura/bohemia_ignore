...
public JsonResult ListadoProveedores(string nombre)
{
    ...
    if (!string.IsNullOrEmpty(nombre))
    {
        proveedores = proveedores
            .Where(p => p.NombreProveedor.StartsWith(nombre));
    }

    var proveedorMostrar = proveedores
        .OrderBy(p => p.Activo == true ? 0 : 1) // 👈 inactivos primero
        .ThenBy(p => p.NombreProveedor)
        .Select(p => new ProveedorVista
        {
    ...
    return Json(new { proveedores = proveedorMostrar });
}
